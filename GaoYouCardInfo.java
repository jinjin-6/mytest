 ...
 */
@Data
public class GaoYouCardInfo {
    private byte[] cardNo = new byte[9]; // 卡号(9字节)
    ...
    public GaoYouCardInfo(byte[] cardData) {
        if (cardData == null || cardData.length != 0x25) {
            throw new IllegalArgumentException("卡数据长度必须为37字节(0x25)");
        }
        int pos = 0;
        // 1. 卡号 (9字节)
        System.arraycopy(cardData, pos, cardNo, 0, 9);
        pos += 9;
        // 2. 密码 (4字节)
